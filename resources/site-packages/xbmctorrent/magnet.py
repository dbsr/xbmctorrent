...
@plugin.route("/download/<uri>/<label>")
def download(uri, label):
    import torrent2http
    base_dlpath = xbmc.validatePath(xbmc.translatePath(plugin.get_setting("dlpath"))) or "."
    dlpath = os.path.join(base_dlpath, label)
    os.mkdir(dlpath)
    if uri.startswith("magnet:") and plugin.get_setting("magnet_boost", bool):
            plugin.log.info("Enabled magnet booster")
            uri = _boost_magnet(uri)
    ...
    plugin.notify('Background download started: {0}'.format(
        torrent2http_options['dlpath']))
    torrent2http.start(**torrent2http_options)
